...
def find_muls(memory):
    return [Mul(left=int(mul[0]), right=int(mul[1])) for mul in re.findall(r"mul\((\d+),(\d+)\)", memory)]
...
def find_muls_with_intervals(memory):
    ...
    return list(chain.from_iterable([find_muls(memory[interval["start"]:interval["end"]]) for interval in intervals]))
